# Reformat Kleinecke et al. (2025) target spreadsheet into BED
...
if (interactive()) {
  arg <- list(
    target_info = "../../resources/41467_2025_62357_MOESM4_ESM.xlsx"
  )
}

# Read in data ---------------------------------------------------------
target_info <- read_excel(arg$target_info) %>%
  select(Chromosome, Start, End, Amplicon_name, Final_set) %>%
  filter(Final_set == 1) %>%
  select(-Final_set)
