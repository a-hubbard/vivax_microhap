...
inp <- list(
  mh_pvgap = "../../results/microhap/MalariaGEN/PvGAP/mg_microhap.csv", 
  mh_kleinecke = "../../results/microhap/MalariaGEN/Kleinecke/mg_microhap.csv", 
  mh_pvgtseq = "../../results/microhap/MalariaGEN/PvGTSeq/mg_microhap.csv", 
...
unique(mh_pvgap$locus)
length(unique(mh_pvgap$locus))
mh_kleinecke <- read_csv(
    inp$mh_kleinecke, 
    col_types = cols(
      .default = col_character(), 
      Lat = col_double(), 
  ...
  select(locus, sample_id, hapseq, Population) %>%
  mutate(panel = "Kleinecke") %>%
  # Filter to diversity loci
  filter(str_detect(locus, "^[0-9]+$"))
unique(mh_kleinecke$locus)
length(unique(mh_kleinecke$locus))
mh_pvgtseq <- read_csv(
    inp$mh_pvgtseq, 
    col_types = cols(
      .default = col_character(), 
  ...
  select(locus, sample_id, hapseq, Population) %>%
  mutate(panel = "PvGTSeq") %>%
  # Filter out DR markers
  filter(str_sub(locus, 1L, 2L) != "pv")
unique(mh_pvgtseq$locus)
length(unique(mh_pvgtseq$locus))
mh_full <- bind_rows(
    mh_pvgap, 
    mh_kleinecke, 
    mh_pvgtseq
...
panels <- c("PvGAP", "Kleinecke", "PvGTSeq")
...
marker_metrics <- marker_metrics %>%
  ...
  mutate(panel_lbl = str_replace(panel_lbl, "Kleinecke", "Kleinecke et al."))
