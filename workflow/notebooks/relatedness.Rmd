---
title: Dcifer Relatedness for Microhaplotype Data
...
inp <- list(
  rel_mat = "../../results/rel_mat.rds", 
  mh_data = 
    "../../results/microhap/MalariaGEN/mg_microhap_filtered4popgen.csv", 
  mg_pop_key = "../../resources/mg_pop_key.csv"
)
out <- list(
  sample_rel = "../../results/sample_rel.csv"
)
```
...
This histogram shows the individual-level relatedness values present in the
dataset. As expected, there are a great many sample pairs with 0 relatedness. 
These are removed to better visualize the remaining pairs.
...
This histogram shows the same thing, except pairs with a relatedness of 0 have 
been removed to allow closer inspection of the remaining values. This figure 
shows that most of the non-zero relatedness pairs have values between 0 and 
0.25, after which point the frequency declines fairly continuously up to 0.5. A 
few pairs have very high relatedness, with values near 1.

Overall, these results make sense. In a geographically-disparate dataset such as 
this one, most sample pairs should have a fairly low relatedness. Also, while we 
expect most pairs are not clonally related, this does occur in low transmission 
settings, so the fact that a few pairs have relatedness at or near 1 is not 
surprising.

# Relatedness Heatmaps

In this section, sample-level relatedness is aggregated to the group level,
starting with MalariaGEN populations.
...
This heatmap should only have color swatches in the lower triangle. This is the
case.
...
This heatmap shows the mean relatedness within and among all sample pairs
corresponding to pairs of MalariaGEN populations. The key for these populations
is given below:
...
This analysis shows, unsurprisingly, that within-population relatedness is much
higher than between-population relatedness. It is particularly high in Latin
America and Oceania. This makes particular sense in the case of Latin America,
which is considered to be isolated compared to other *P. vivax* populations.

The between-population relatedness patterns also conform to expectations:
relatedness is higher between Africa and West Asia and between East south-east
Asia and Oceania than between other pairs. These pairs are also in the closest
geographic proximity to each other relative to the others.

## Geography-Level

In this section, relatedness is analyzed at the level of more localized
geographies, defined manually by merging some sample sites and removing others
to create a set of distinct geographies that each has an acceptable sample size.
...
This heatmap shows the relatedness within and between geographies, sorted on a 
rough east-west axis. Exactly as one would expect, relatedness is higher within
versus between populations. Indeed, relatedness between geographies from the
same populations is comparable to relatedness within populations, which
indicates a high degree of transmission and gene flow within these populations 
(i.e., these results support the population definitions created by MalariaGEN).

One can also observe higher relatedness between Afghanistan and the locations in 
Ethiopia, and between Indonesia and the Southeast Asia locations, than between 
other geography pairs. This makes sense, as these locations are all from the 
same regions, and is also consistent with the population-level results, above.

### Southeast Asia

One of the desired capabilities of this panel is to be able to identify
within-region population structure, so in this section the geographies from
Southeast Asia are examined in more detail.
...
This heatmap shows the mean relatedness within and between the geographies from
Southeast Asia. The spread of mean relatedness is fairly low within this region,
providing limited ability to identify patterns. For this reason, another metric,
the fraction of significantly-related sample pairs, was examined as a
complement.
...
This heatmap shows the fraction of sample pairs that are significantly related
within and between the geographies from Southeast Asia. Ranging from roughly 0.5
to 0.7, this metric provides a somewhat improved ability to discriminate between
geography pairs than mean relatedness.

The patterns evident in this figure do not conform to a simple pattern of
isolation-by-distance. Cambodia and Ho Chi Min are as closely-related as Ho Chi
Min and Krong Pa, which are much closer. In general, Ho Chi Min is
closely-related to the other locations in the region, in some cases with higher
relatedness than was found within that location. This might be explained by the
fact that Ho Chi Min is a large city with lots of travel in and out.

However, with that hypothesis, one would expect the within-population 
relatedness to be lower in Ho Chi Min than in other geographies, because greater 
human movement in and out of the city should increase parasite diversity. The 
opposite pattern was observed, however - Ho Chi Min has the highest 
within-population relatedness of any of these locations.
