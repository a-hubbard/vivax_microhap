...
The distribution of *p*-values is given above. It looks somewhat
uniform, suggesting there may not be linkage in the dataset.
...
To provide an overall measure of linkage disequilibrium, LD was also
tested using the $\bar{r}_{d}$ measure. This was used in favor of the
more common index of association because the index of association is a
ratio of the variance observed between individuals at each locus and the
variance expected under conditions of linkage equilibrium. This measure
increases with the number of loci, so $\bar{r}_{d}$ was developed as an
alternative that approximates the index of association while avoiding
this limitation.
...
```
    ...
    ##         Ia       p.Ia      rbarD       p.rD 
    ## 2.34848324 0.96000000 0.04739847 0.96000000
...
With that aside, this method of estimating overall LD shows agreement
with the $T_2$ statistic, in that no significant linkage is estimated to
be present.
